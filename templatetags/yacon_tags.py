...
def _render_block(context, pagetranslation, blocks, tag_name, tag_parameters):
...
def block_by_key(context, key):
    ...
    pt = context['pagetranslation']
    blocks = pt.blocks.filter(block_type__key=key)

    return _render_block(context, pt, blocks, 'block_by_key', 'key:%s' % key)
